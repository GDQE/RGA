        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', height: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <span style={{ fontSize: 13, color: C.textSub }}>
              <strong style={{ color: C.accentMid }}>{answered}</strong>/{questions.length} إجابة
            </span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: isLow ? C.dangerBg : C.accentGhost, border: '1px solid ' + (isLow ? C.danger + '55' : C.accentLight + '55'), borderRadius: 10, padding: '6px 14px' }}>
              <span style={{ fontSize: 19, fontWeight: 900, fontFamily: 'monospace', color: isLow ? C.danger : C.accent }}>
                {String(mins).padStart(2,'0')}:{String(secs).padStart(2,'0')}
              </span>
            </div>
            <button onClick={tryFinish} style={{ padding: '7px 18px', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: font, background: answered < questions.length ? C.border : C.success, color: answered < questions.length ? C.textMuted : '#fff', border: 'none' }}>
              إنهاء الاختبار
            </button>
          </div>
        </div>
        ...
        <div style={{ background: C.surface, borderRadius: 18, padding: '32px 36px', boxShadow: C.shadowMd, border: '1px solid ' + C.border }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
            ...
            <button onClick={() => toggleFlag(q.id)} style={{ padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600, background: flagged.has(q.id) ? C.warningBg : 'transparent', border: '1px solid ' + (flagged.has(q.id) ? C.warning : C.border), color: flagged.has(q.id) ? C.warning : C.textMuted, fontFamily: font }}>
              {flagged.has(q.id) ? 'ملغ للمراجعة' : 'علم للمراجعة'}
            </button>
          </div>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <div key={i}
                  ...
                  <div style={{ width: 30, height: 30, borderRadius: 8, flexShrink: 0, border: '2px solid ' + (sel ? C.accentMid : C.border), background: sel ? C.accentMid : C.surface, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: sel ? '#fff' : C.textMuted }}>
                    {sel ? 'v' : ['a','b','c','d'][i]}
                  </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 28 }}>
            ...
            {currentQ < questions.length-1
              ...
              : <button onClick={tryFinish} style={{ padding: '10px 26px', borderRadius: 10, fontSize: 13, cursor: 'pointer', background: answered < questions.length ? C.border : C.success, color: answered < questions.length ? C.textMuted : '#fff', border: 'none', fontWeight: 800, fontFamily: font }}>إنهاء الاختبار</button>
            }
          </div>
        </div>
      </div>
      ...
      {confirmOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: C.surface, borderRadius: 20, padding: '36px 40px', maxWidth: 400, width: '100%', boxShadow: C.shadowLg, direction: 'rtl', textAlign: 'center' }}>
            <h3 style={{ margin: '0 0 10px', fontSize: 19, fontWeight: 800, color: C.text, fontFamily: font }}>تاكيد إنهاء الاختبار</h3>
