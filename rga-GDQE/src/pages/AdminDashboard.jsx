        ...
        <nav style={{ flex: 1, padding: '16px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {[
            ...
            <button key={item.id} onClick={() => setTab(item.id)} style={{
              ...
              borderRight: tab === item.id ? '3px solid #2E7D52' : '3px solid transparent'
            }}>
                ...
                {stats.specialtyStats.map(sp => (
                  <div key={sp.name} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: C.bg, borderRadius: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ background: C.border, borderRadius: 4, height: 6, display: 'flex', overflow: 'hidden' }}>
                        <div style={{ height: '100%', background: C.success, width: `${sp.passRate}%`, transition: 'width 0.5s' }} />
                      </div>
                    </div>
                ...
                <>
                    ...
                    <thead>
                      <tr style={{ background: '#F8FAFC', borderBottom: `2px solid ${C.border}` }}>
                        {['#', 'الاسم', 'الشركة', 'التخصص', 'الدرجة', 'النتيجة', 'التاريخ', 'إجراءات'].map(h => (
                          <th key={h} style={{ padding: '12px 16px', textAlign: 'right', fontSize: 12, fontWeight: 700, color: C.textSub, fontFamily: font, whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}` }}
                          ...
                          <td style={{ padding: '12px 16px', fontSize: 13, color: C.textSub, fontFamily: font }}>
                            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              {SPECIALTY_ICONS[r.candidates?.specialty]} {r.candidates?.specialty}
                            </span>
                          </td>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button onClick={() => navigate(`/admin/result/${r.id}`)} style={{ padding: '5px 12px', borderRadius: 6, border: `1px solid ${C.accentMid}`, background: C.accentGhost, color: C.accentMid, fontSize: 12, cursor: 'pointer', fontFamily: font }}>تفاصيل</button>
                              <button onClick={() => setDeleteConfirm(r.id)} style={{ padding: '5px 12px', borderRadius: 6, border: `1px solid ${C.danger}`, background: C.dangerBg, color: C.danger, fontSize: 12, cursor: 'pointer', fontFamily: font }}>حذف</button>
                            </div>
                          </td>
                        </tr>
                      ))}
                  ...
                  {totalPages > 1 && (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, padding: '16px', borderTop: `1px solid ${C.border}` }}>
                      <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} style={{ padding: '6px 14px', borderRadius: 7, border: `1px solid ${C.border}`, background: 'transparent', cursor: page===1 ? 'not-allowed' : 'pointer', color: page===1 ? C.textMuted : C.textSub, fontSize: 13, fontFamily: font }}>← السابق</button>
                      <span style={{ fontSize: 13, color: C.textSub, fontFamily: font }}>صفحة {page} من {totalPages}</span>
                      <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={page===totalPages} style={{ padding: '6px 14px', borderRadius: 7, border: `1px solid ${C.border}`, background: 'transparent', cursor: page===totalPages ? 'not-allowed' : 'pointer', color: page===totalPages ? C.textMuted : C.textSub, fontSize: 13, fontFamily: font }}>التالي →</button>
                    </div>
                  )}
                </>
              )}
            ...
            <div style={{ background: C.surface, borderRadius: 16, padding: 24, boxShadow: C.shadow, border: `1px solid ${C.border}` }}>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  ...
                  <Legend wrapperStyle={{ fontFamily: font }} />
                  <Tooltip contentStyle={{ fontFamily: font }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div style={{ background: C.surface, borderRadius: 16, padding: 24, boxShadow: C.shadow, border: `1px solid ${C.border}` }}>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={stats.specialtyStats} layout="vertical">
                  ...
                  <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fontFamily: font }} width={100} />
                  <Tooltip contentStyle={{ fontFamily: font, direction: 'rtl' }} />
                  <Bar dataKey="avgScore" fill={C.accentMid} name="متوسط الدرجة" radius={[0,4,4,0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: C.surface, borderRadius: 16, padding: '32px 36px', maxWidth: 360, width: '100%', boxShadow: C.shadowLg, direction: 'rtl', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button onClick={() => handleDelete(deleteConfirm)} style={{ flex: 1, padding: '10px', borderRadius: 8, cursor: 'pointer', background: C.danger, color: '#fff', border: 'none', fontWeight: 700, fontFamily: font }}>حذف</button>
            </div>
          </div>
        </div>
      )}
